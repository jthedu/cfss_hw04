...
ggplot(data = regional_data, mapping = aes(x = life_expectancy, y = popdensity, color = country_name, group = country_name)) +
  geom_line() +
#  facet_wrap(fct_rev(country_name) ~.) +
...
ggplot(data = regional_data, mapping = aes(x = totalpopgrow/100, y = popdensity, color = country_name, group = country_name)) +
  geom_line() +
#  facet_wrap(fct_rev(region) ~.) +
    labs(title = "Total Population Growth vs. Population Density, 1960-2020", x = "Percent of total population growth", y = "Population density (people per sq. km)", caption = "Source: The World Bank", color = "Country") +
    scale_x_continuous(labels = scales::percent) 
